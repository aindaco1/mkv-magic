...
identifier—and current schema v16 preserves that boundary. v1-v15 workflows
...
Schema v13 adds **If useful: Normalize commentary names**. Recognition is
limited to audio and subtitle tracks whose native commentary role is already
set or whose current name contains the distinct case-insensitive word
`commentary`. Within audio and subtitles independently, inspected order maps to
`Commentary`, `Commentary #2`, and so on. Already correct names are skipped; a
name that must change requires a UID unique across the complete track table.
Each ephemeral edit changes only the name and keeps language, every role and
playback flag, technical facts, nested chapters, attachments, tags, and packets
unchanged. When name normalization and flag marking both target one track, the
compiler merges them by stable UID into one reviewed edit and one property pass.
The portable recipe and sanitized queue history retain neither original nor
resolved names, UIDs, counts, paths, or media identity.

Schema v14 adds **If useful: Mark forced subtitles**. Compilation considers
only subtitle tracks whose forced role is currently unset and whose inspected
name contains `forced` as a distinct case-insensitive word; audio/video tracks,
already forced subtitles, and substring lookalikes are unchanged. Every match
must have a UID unique across the complete track table. The ephemeral edit sets
only `flag-forced=1`, preserving the name, canonical language meaning, default,
enabled and accessibility roles, technical facts, packets, chapters,
attachments, and tag policy. Overlapping commentary-role, commentary-name, and
...
Schema v16 adds **If useful: Mark audio-description tracks**. Compilation
considers only audio tracks whose visual-impaired role is currently unset and
whose inspected name contains a clear case-insensitive phrase: `audio
description`, `audio described`, `descriptive audio`, `visual impaired`, or
`visually impaired`; the equivalent compact spellings are also recognized.
The ambiguous standalone abbreviation `AD`, non-audio tracks, already marked
tracks, and substring lookalikes are unchanged. Every match must have a UID
unique across the complete track table. The ephemeral edit sets only
`flag-visual-impaired=1`, preserving the name, canonical language meaning,
default, forced, enabled and every other role, layout, technical facts, packets,
chapters, attachments, and tag policy. All role/name intent targeting one UID
becomes one edit in the same fail-closed property pass. The portable recipe and
sanitized History retain only the action and count-level outcome, never resolved
UIDs or private names.
...
1. Release only an immutable, signed, annotated `vMAJOR.MINOR.PATCH` tag that resolves to the reviewed `main` commit and validates against the repository's pinned allowed signers. Derive the positive signed-32-bit `CFBundleVersion` from that signed tag's Unix tagger timestamp, require it to increase beyond the accepted prior build, and keep the result stable across workflow reruns. Before any release build, live API evidence must prove the repository is public, immutable releases are enabled, and an active no-bypass `v*` tag ruleset blocks tag update and deletion. Those controls were enabled and passed the live repository preflight on August 25, 2026; every release must reverify them rather than relying on that observation.
...
only the naming intent, never a source or generated filename; current schema v16
retains that boundary and strictly migrates v1-v15 without allowing an older
...
The current instrumented result is 88.64% (22,587 of 25,483 lines). The AppKit
