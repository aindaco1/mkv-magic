...
final class SavedWorkflowStoreTests: XCTestCase {
    ...
    func testPortableFileRoundTripsClearAllTagsIntentWithoutMediaFacts() throws {
        ...
        XCTAssertTrue(json.contains("\"schemaVersion\" : 16"))
    ...
    func testPortableImageCleanupStoresNoAttachmentFacts() throws {
        let workflow = SavedWorkflow(
            name: "Remove images",
    ...
    func testPortableCommentaryNameNormalizationStoresNoResolvedNamesOrTracks() throws {
        let workflow = SavedWorkflow(
            name: "Simple commentary labels",
    ...
    func testPortableAudioDescriptionMarkingStoresNoResolvedTrackFacts() throws {
        let workflow = SavedWorkflow(
            name: "Audio description roles",
            steps: [SavedWorkflowStep(action: .markAudioDescriptionTracks)]
        )
        ...
        XCTAssertFalse(json.contains("trackUID"))
        XCTAssertFalse(json.contains("visualImpairedCount"))
        XCTAssertFalse(json.contains("trackName"))
        XCTAssertFalse(json.contains("sourceURL"))
        XCTAssertEqual(try JSONSavedWorkflowStore.decodePortableFile(encoded), workflow)
    }

    func testVersionFifteenMigratesButCannotClaimVersionSixteenAudioDescriptionMarking() throws {
        let valid = Data(
            #"{"id":"B989848F-887B-4861-AF7E-ADE3E6E64883","schemaVersion":15,"name":"SDH","steps":[{"id":"22D43583-1382-4778-A4EC-D8618D3A6A4B","isEnabled":true,"action":"markSDHSubtitles"}]}"#
                .utf8
        )
        XCTAssertEqual(
            try JSONSavedWorkflowStore.decodePortableFile(valid).schemaVersion,
        ...
        let invalid = Data(
            #"{"id":"B989848F-887B-4861-AF7E-ADE3E6E64883","schemaVersion":15,"name":"Invalid description backport","steps":[{"id":"22D43583-1382-4778-A4EC-D8618D3A6A4B","isEnabled":true,"action":"markAudioDescriptionTracks"}]}"#
                .utf8
        )
        XCTAssertThrowsError(try JSONSavedWorkflowStore.decodePortableFile(invalid)) {
            XCTAssertEqual($0 as? SavedWorkflowStoreError, .unsupportedSchema)
    ...
    func testVersionTwelveMigratesButCannotClaimVersionThirteenNameNormalization() throws {
        let valid = Data(
            #"{"id":"B989848F-887B-4861-AF7E-ADE3E6E64883","schemaVersion":12,"name":"Commentary","steps":[{"id":"22D43583-1382-4778-A4EC-D8618D3A6A4B","isEnabled":true,"action":"markCommentaryTracks"}]}"#
                .utf8
        )
        XCTAssertEqual(
            try JSONSavedWorkflowStore.decodePortableFile(valid).schemaVersion,
        ...
        let invalid = Data(
            #"{"id":"B989848F-887B-4861-AF7E-ADE3E6E64883","schemaVersion":12,"name":"Invalid name backport","steps":[{"id":"22D43583-1382-4778-A4EC-D8618D3A6A4B","isEnabled":true,"action":"normalizeCommentaryNames"}]}"#
                .utf8
        )
        XCTAssertThrowsError(try JSONSavedWorkflowStore.decodePortableFile(invalid)) {
            XCTAssertEqual($0 as? SavedWorkflowStoreError, .unsupportedSchema)
    ...
    func testVersionTenMigratesButCannotClaimVersionElevenImageCleanup() throws {
        let valid = Data(
            #"{"id":"B989848F-887B-4861-AF7E-ADE3E6E64883","schemaVersion":10,"name":"Tags","steps":[{"id":"22D43583-1382-4778-A4EC-D8618D3A6A4B","isEnabled":true,"action":"clearAllTags"}]}"#
                .utf8
        )
        XCTAssertEqual(
            try JSONSavedWorkflowStore.decodePortableFile(valid).schemaVersion,
        ...
        let invalid = Data(
            #"{"id":"B989848F-887B-4861-AF7E-ADE3E6E64883","schemaVersion":10,"name":"Invalid image cleanup backport","steps":[{"id":"22D43583-1382-4778-A4EC-D8618D3A6A4B","isEnabled":true,"action":"removeImageAttachments"}]}"#
                .utf8
        )
        XCTAssertThrowsError(try JSONSavedWorkflowStore.decodePortableFile(invalid)) {
            XCTAssertEqual($0 as? SavedWorkflowStoreError, .unsupportedSchema)
