...
- Selecting an inspected Matroska video enables **Add Subtitle…** and an SRT,
  ASS, or SSA file picker.
...
- Existing deterministic cleanup suggestions are disclosed but not silently
  applied. The user must run **Clean Subtitle…** first to opt into text cleanup.
...
4. `mkvmerge` receives an exact absolute executable URL, `--abort-on-warnings`,
   ...
   non-attachment track followed by the new text subtitle. No shell or encoder
   is used.
5. Before commit and again after reopen, verification requires a nonempty
   ...
   facts. It also requires one final S_TEXT/UTF8, S_TEXT/ASS, or S_TEXT/SSA track
   matching the reviewed input and its language, name, and flags, plus a fresh
   segment UID. ASS/SSA is re-extracted before commit and after reopen for a
   payload audit.
6. The video, external subtitle, and destination are security-scoped only for the
...
- Output-verifier tests accept only one reviewed text subtitle added last and
  reject a mutation to an existing track; shared fixtures also carry chapters
  and an attachment through the verification contract.
- A real bundled-tool test creates a Matroska source with FFmpeg, decodes a
  ...
  `mkvextract`.
- Real bundled-tool integrations cover Windows-1252 SRT, modern ASS, legacy SSA,
  and app-history execution while preserving both selected inputs and excluding
  personal paths and subtitle text from history.
...
- The current full local gate passes 160 tests with eight intentionally skipped
  bundled-tool tests, coverage collection, AddressSanitizer,
  ...
  DMG verification. All eight bundled-tool integrations also pass separately
